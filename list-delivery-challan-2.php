...
if($_POST['act']=='ust')
{
   ...
   if($id != '' && $status != '')
   {
      $rs_UpdReg = mysqli_query($conn,"update delivery_challan set status = 'Received' where id = '$id'");
   }
   ?>
<?
    $rs_SelReg = mysqli_query($conn,"select * from delivery_challan where id = '$id'");
...
$.ajax({
url:'list-delivery-challan-2.php',
...
function drwStatus(id, St){
   if(St=='1'){
      document.getElementById("spSt"+id).innerHTML = '<div class="mt-2"><span class="label-paid rounded-1">Received</span></div>';
   } 
}
 </script>
...
header('Location:list-delivery-challan-2.php?alert_msg='.$alert_msg);
}
else
{
...
$select_delivery_challan=mysqli_query($conn,"select * from delivery_challan where 1=1 and returnable_type='Non-Returnable' $subquery"); 
...
<h6 class="mb-0 text-uppercase">List Not-Returnable Delivery Challan</h6>

<div class="ms-auto">
<div class="col">
<!-- Button trigger modal -->
<a href="add-delivery-challan-2.php" class="btn btn-primary">Add Delivery Challan</a>
<!-- Modal -->
</div>
</div>
</div>
...
<form action="list-delivery-challan-2.php" class="" method="post">
...
<a href="add-delivery-challan-2.php?id=<?=$id; ?>" tooltip="Edit" class="btn btn-add btn-sm"><i class="bx bxs-edit"></i></a>
<a href="#" class="ms-2" data-toggle="modal" tooltip="Delete"  data-target="#customer2" onClick="if(confirm('Are you sure want to delete this?')) { window.location.href='list-delivery-challan-2.php?act=delete&id=<?=$id ?> ' }"><i class="bx bxs-trash"></i></a>
</div>
<div class="d-flex mt-2">
<a href="view-delivery-challan-2.php?id=<?=$id; ?>" tooltip="View" class="btn btn-add btn-sm"><i class="lni lni-eye"></i></a>
<a href="export-delivery-challan-2.php?id=<?=$id; ?>" tooltip="Print" target="_blank" class="btn btn-add btn-sm ms-2"><i class="bx bxs-printer"></i></a>
</div>
