...
$status='Approved';
$insert_product=mysqli_query($conn,"insert into order_confirmation set quotation_id = '$ID', customer_id = '$customer', customer_name = '$customer_name', enquiry_num = '$enquiry_num', refered_by = '$refered_by', invoice_date = '$invoice_date', company_name='$company_name', terms_condition='$terms_condition', company_address= '$company_address', customer_type='$customer_type', mobile='$mobile', products_count='$products_count', status='$status', created_by=".$_SESSION['UID']."");
...
$total_order_amount = round(($total_order_amount+ $total_amount[$i]),2);
      ...
      $rs_InsValues=mysqli_query($conn,"insert into ordered_products set sales_id='$sales_id', product_name='$product_name[$i]', brand_name='$brand_name[$i]', rate='$rate[$i]', quantity = '$quantity[$i]', package_amount = '$package_amount', product_amount = '$product_amount', gst = '$gst[$i]', remarks='$remarks[$i]', estimated_delivery = '$estimated_delivery[$i]', package_percent = '$pkg[$i]', gst_amount = '$Gst_amount', total_amount = '$total_amount[$i]', created_datetime = '$currentTime', created_by=".$_SESSION['UID']."");
   }
}
if($freight_charges!=""){
...
$update_total=mysqli_query($conn,"update order_confirmation set product_amount='$total_product_amount', gst_amount='$gst_amount', freight_charges='$freight_charges', freight_gst='$freight_gst', total_freight='$total_freight',  total_order_amount='$total_order_amount', total_package_amount='$total_package_amount', created_datetime = '$currentTime' where id='$sales_id'");

$update_quotation=mysqli_query($conn,"update quotation set status='Approved', modified_datetime = '$currentTime' where id='$ID'");  
if($update_total && $update_quotation){
         $msg = 'Sales Added Successfully';
       header('Location:add-sales.php?id='.$sales_id.'&act=edit&msg='.$msg."#preview-btn");
...
$products_count=count($product_name);
$status='Approved';
$update_product=mysqli_query($conn,"update order_confirmation set customer_id = '$customer', customer_name = '$customer_name', enquiry_num = '$enquiry_num', refered_by = '$refered_by', invoice_date = '$invoice_date', company_name='$company_name', terms_condition='$terms_condition', company_address= '$company_address', customer_type='$customer_type', mobile='$mobile', products_count='$products_count', modified_datetime=".$_SESSION['UID']." where id='$ID' ");

$quotation_ProductDelete = mysqli_query($conn,"delete from ordered_products where sales_id ='$ID' ");
...
$total_order_amount = round(($total_order_amount+ $total_amount[$i]),2);
      ...
      $rs_UpdateValues=mysqli_query($conn,"insert into ordered_products set sales_id='$ID', product_name='$product_name[$i]', brand_name='$brand_name[$i]', rate='$rate[$i]', quantity = '$quantity[$i]', package_amount = '$package_amount', product_amount = '$product_amount', gst = '$gst[$i]', remarks='$remarks[$i]', estimated_delivery = '$estimated_delivery[$i]', package_percent = '$pkg[$i]', gst_amount = '$Gst_amount', total_amount = '$total_amount[$i]', total_dispatch = '$total_dispatch[$i]', created_datetime = '$currentTime', created_by=".$_SESSION['UID']."");
   }
}
if($freight_charges!=""){
$gst_freight = round(($freight_charges * $freight_gst) / 100,2);
...
$total_order_amount=$total_freight+$total_order_amount;
$update_sales=mysqli_query($conn,"update order_confirmation set product_amount='$total_product_amount', gst_amount='$gst_amount', freight_charges='$freight_charges', freight_gst='$freight_gst', total_freight='$total_freight', total_order_amount='$total_order_amount', total_package_amount='$total_package_amount', created_datetime = '$currentTime' where id='$ID'");
...
header('Location:add-sales.php?id='.$ID.'&act=edit&msg='.$msg."#preview-btn");
}else
   {
      $alert_msg = 'Could not able to add try once again!!!';     
   }  
  ...
  if($Act=="edit"){
    $table_1="order_confirmation";
    $table_2="ordered_products where sales_id = ".$ID."";
  }
  else{
    $table_1="quotation";
    $table_2="quotation_product where quotation_id = ".$ID."";
  }
$sel_values=mysqli_query($conn,"select * from $table_1 where id = '$ID'"); 
...
<div class="card-body p-5">
  ...
  <h5 class="text-uppercase mb-0"><? if($Act == "edit") { echo "Update Sales"; } else echo "Move to Sales";?></span></h5>
 <?if($Act== "edit"){?> <a href="list-sales.php" class="btn btn-danger">Back</a><?}else{?> <a href="list-quotation.php" class="btn btn-danger">Back</a><?}?>
  </div>
<hr>
...
<label for="inputLastName" class="form-label mt-20">Customer PO No.</label>
<input type="text" class="form-control" name="enquiry_num" value="<?=$enquiry_num;?>" placeholder="Customer PO No.">
</div>

<div class="col-xl-3 mt-3">
<label for="inputLastName" class="form-label mt-20">Job Card No</label>
<input type="text" class="form-control" name="refered_by" value="<?=$refered_by;?>" placeholder="Job Card No">
</div>

<div class="col-xl-2 mt-3 w-23">
<label for="inputLastName" class="form-label mt-20">PO Date</label>
<input type="date" class="form-control" name="invoice_date" value="<? if($Act == "edit") { echo $invoice_date; } else echo $currentDate;?>">
</div>

<!-- <div class="col-xl-2 mt-3 w-23">
<label for="inputLastName" class="form-label mt-20">Estimated Delivery Date</label>
<input type="date" class="form-control" name="estimated_delivery" value="<?=$estimated_delivery;?>" required>
</div> -->

</div>
<div class="bg-3f663a text-center mt-4 mb-3 rounded-1">
  <h5 class="text-white py-1">Add Product</h5>
</div>
<div class="px-2" id="Regular" >
<div id="frm_scents" class=" sml-product">
<? if($CountQtn >>0){ ?>
...
<? 
$SNo = 0;
$sel_product=mysqli_query($conn,"select * from  $table_2 "); 
...
<div class="w-6">
<input type="number" class="form-control p-qnt" id="quantity<?=$SNo?>" onchange="getTotal(<?=$SNo?>);addRate(<?=$SNo?>,this.value,'quantity');"  name="quantity[]" placeholder="Qty" min="<?=$total_dispatch;?>"  value="<?=$quantity;?>" required>
...
<?}else{ ?>
<div class="w-5 text-end" id="remScnt" onclick="removeCont(this);"><a type="button" style="width: auto;" class="pe-1" tooltip="Remove Product"><img src="assets/images/Our/minus.png" width="27px;"></a></div>
<? } } ?>
...
<input type="submit" name="Submit" class="btn btn-primary px-3" value="<? if($Act == "edit") { echo "Update Sales"; } else echo "Move to Sales";?>" >
<? if($Act=="edit"){ ?>
<div class="col-12 mt-3"  id="preview-btn"> 
<a href="preview-sales.php?id=<?=$ID; ?>" class="btn btn-warning px-3 ">Preview Invoice</a></div>
<?}?>
</div> 
</div>
</div>
...
id=<? if($Act == "edit") { echo $quotation_id; } else echo $ID;?>;
$.ajax({
url: "ajax-price-log.php", 
type: "POST",
data: "id="+id+"&product_name="+product_name+"&table_type="+table_type+"&total_value="+total_value+"&type="+type+"&new_value="+val+"&category=1",
success: function(result){
